...
<div class="simple-cta-layout bg-[#fafafa] px-6 py-10 md:px-8 md:py-[60px] lg:px-10 lg:py-10 xl:px-20">
	<div class="ff-c-container flex flex-col justify-between gap-x-6 gap-y-6 md:flex-row">
		...
		<?php if ( $simple_link ) : ?>
			<div class="link-container ff-button-secondary my-auto md:flex">
				<a class="block w-full rounded-sm border-[1.5px] border-ada_red-50 px-4 pb-3 pt-[14px] text-center font-geova text-lg font-medium uppercase text-ada_red-50 md:w-auto" href="<?php echo esc_url( $simple_link['url'] ?? '#' ); ?>" target="<?php echo esc_attr( $simple_link['target'] ?? '' ); ?>"> <?php echo esc_html( $simple_link['title'] ?? '' ); ?></a>
			</div>
		<?php endif; ?>
	</div>
</div>
