...
jQuery( function( $ ) {
	...
	$( '#loginPopup' ).on( 'submit', function( e ) {
		...
		if ( ! /^[^@]+@[^@]+\.[^@]+$/.test( emailVal ) ) {
			...
			$email.after( '<div class="login-error text-red-500 text-sm mt-1">Please enter a valid email address.</div>' );
			return;
		}
		...
		if ( ! passwordVal ) {
			...
			$password.after( '<div class="login-error text-red-500 text-sm mt-1">Please enter your password.</div>' );
			return;
		}
		...
		loginFunction( emailVal, passwordVal, function( errorType, errorMsg ) {
			...
			if ( errorType === 'user' ) {
				$email.addClass( 'border-red-500' );
				$emailLabel.addClass( 'text-red-500' );
				$email.after( '<div class="login-error text-red-500 text-sm mt-1">' + errorMsg + '</div>' );
			} else if ( errorType === 'password' ) {
				$password.addClass( 'border-red-500' );
				$passwordLabel.addClass( 'text-red-500' );
				$password.after( '<div class="login-error text-red-500 text-sm mt-1">' + errorMsg + '</div>' );
			}
		}, $btn );
	} );
